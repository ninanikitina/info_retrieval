...
class GenerateSnippets:
    def __init__(self):
        ...
        termsList = self.a['all_content'][4]
        doc = self.b['content'][4]
        #f = open("C:\\Users\\steph\\source\\repos\\info_retrieval\\Project1\\myPickle", "wb")
        #pickle.dump(termsList, f)
        #f.close()

        #f = open("C:\\Users\\steph\\source\\repos\\info_retrieval\\Project1\\preprocessed_files\\wiki_index.pickle", "rb")
    ...
    def cosine(self, query, sentence, termsList): 
        ...
        for t in termsList:
            ...
            magQ.append(q**2)
            magS.append(s**2)
        num = (math.sqrt(sum(magQ)) * math.sqrt(sum(magS)))
        if num == 0:
            return 0
        else:
            return sum(prod) / (math.sqrt(sum(magQ)) * math.sqrt(sum(magS)))
